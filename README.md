...
| 📊 **`pyats_run_show_command`** | `device_name`: str<br>`command`: str | Executes a show command on a device and returns raw output. Validates command safety (no pipes, redirects, or dangerous keywords) | General device interrogation - Run any show command and get CLI output |
...
| 🏓 **`pyats_ping_from_network_device`** | `device_name`: str<br>`command`: str | Executes a ping command from a network device (e.g., 'ping 1.1.1.1' or 'ping 1.1.1.1 repeat 100') and returns raw output | Connectivity testing - Verify reachability from the device's perspective |
